...
export default function WorkspaceGithubSettingsPage() {
	...
	useEffect(() => {
		setGitAuthorName(workspace?.gitAuthorName ?? "");
		setGitAuthorEmail(workspace?.gitAuthorEmail ?? "");
	...
	const saveGitSettings = async () => {
		...
		try {
			const response = await fetch("/api/workspaces/saveGithub", {
				method: "POST",
				headers: {
					"Content-Type": "application/json",
					Authorization: `Bearer ${user.refresh_token}`,
							...
							<div className="flex min-w-0 flex-col gap-1">
								<p className="text-xs text-grayscale-11">
									{workspace?.githubAppInstallationId
										? `Connected${workspace.githubAppInstallationAccountLogin ? ` to ${workspace.githubAppInstallationAccountLogin}` : ""}.`
