...
var updateTaskStatus = /* @__PURE__ */ __name(async (taskId, status) => {
...
var processEventTask = task({
  ...
  run: /* @__PURE__ */ __name(async (payload) => {
    ...
    try {
      ...
      await updateTaskStatus(task2.id, "failed");
      throw error;
    }
  }, "run")
});
...
var cloneFactoryRepositories = /* @__PURE__ */ __name(async (sandbox, taskId, factory, envs) => {
  ...
  await runSetupStep(
    ...
    () => sandbox.commands.run(
      [
        ...
        ...repositories.map(
          (repository) => buildCloneRepositoryCommand(
            repository,
            Boolean(envs.GITHUB_ACCESS_TOKEN)
          )
        )
      ].join("\n"),
      { timeoutMs: 10 * 60 * 1e3, envs }
    ),
    { timeoutMs: 11 * 60 * 1e3 }
  );
}, "cloneFactoryRepositories");
var setupRepositoryGithubAuth = /* @__PURE__ */ __name(async (sandbox, taskId, factory) => {
  ...
  return envs;
}, "setupRepositoryGithubAuth");
var buildGithubAuthNoticeCommand = /* @__PURE__ */ __name(() => [
...
var buildGithubAuthHeaderCommand = /* @__PURE__ */ __name(() => [
  'if [ -n "$GITHUB_ACCESS_TOKEN" ]; then',
  '  GITHUB_AUTH_HEADER="$(printf "x-access-token:%s" "$GITHUB_ACCESS_TOKEN" | base64 | tr -d "\\n")"',
  "fi"
].join("\n"), "buildGithubAuthHeaderCommand");
var getFactoryGithubAuthEnvs = /* @__PURE__ */ __name(async (factory) => {
...
}, "getFactoryGithubAuthEnvs");
var validateGithubToken = /* @__PURE__ */ __name(async (sandbox, envs) => {
  let authStderr = "";
  try {
...
var buildCloneRepositoryCommand = /* @__PURE__ */ __name((repository, hasGithubAccessToken) => {
  const path = normalizeRepositoryPath(repository.path) ?? getRepositoryDefaultPath(repository.url);
...
var getGithubHttpsCloneUrl = /* @__PURE__ */ __name((url) => {
  ...
  return void 0;
}, "getGithubHttpsCloneUrl");
var getRepositoryDefaultPath = /* @__PURE__ */ __name((url) => {
  const lastSegment = url.split("/").pop()?.trim() || "repository";
  return normalizeRepositoryPath(lastSegment.replace(/\.git$/i, "")) ?? "repository";
}, "getRepositoryDefaultPath");
var normalizeRepositoryPath = /* @__PURE__ */ __name((value) => {
  if (!value) {
    return void 0;
  }
  const segments = value.trim().replaceAll("\\", "/").split("/").filter(
...
var runCodexExec = /* @__PURE__ */ __name(async (sandbox, task2, factory, prompt, options = {}, agentId) => {
  ...
  try {
    ...
    console.log(result);
    if (!wasInterrupted) {
      await updateTaskStatus(task2.id, "idle");
    }
  } catch (error) {
